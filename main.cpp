...
int main(int argc,char* arg[])
{
    ...
    sort(searchMethodVectorPotential.begin(),searchMethodVectorPotential.end());
    vector<string> paramANDsearchMethod;
    set_intersection(parameters.begin(),parameters.end(),searchMethodVectorPotential.begin(),searchMethodVectorPotential.end(), inserter(paramANDsearchMethod, paramANDsearchMethod.begin()));
    ...
    // Check for the flags
    auto parametersItr = find(parameters.begin(),parameters.end(), "printState");
    ...
    char* path = arg[2];
    cout<< "Configuration: input path: "<< path << " printState: "<<printStates<<" printVector: "<<printVector<< " loopTime: "<<loopTime<<" search method: "<<searchMethod <<endl;
    ...
    // change pl (vector<PropertyAssignment*>) to propertyList (vector<PropertyAssignment>).
    ...
    for (size_t j = 0; j <  propertyList.size(); ++j) {
        if(propertyList[j].get_name() == "CliqueNumber"){
            cout<<"Core " <<j <<" "<<propertyList[j].get_name()<<" "<<propertyList[j].get_operatorSign()<<" "<<propertyList[j].get_value()<<endl;
            if (propertyList[j].get_operatorSign()==">"){
                CliqueNumber_AtLeast_DynamicCore *cliqueNumberGreaterThanOrEqualDynamicCore = new CliqueNumber_AtLeast_DynamicCore(propertyList[j].get_value()+1);
                dynamicKernel.addCore(*cliqueNumberGreaterThanOrEqualDynamicCore);
            } else if (propertyList[j].get_operatorSign()==">="){
                CliqueNumber_AtLeast_DynamicCore *cliqueNumberGreaterThanOrEqualDynamicCore = new CliqueNumber_AtLeast_DynamicCore(propertyList[j].get_value());
                dynamicKernel.addCore(*cliqueNumberGreaterThanOrEqualDynamicCore);
            }else{

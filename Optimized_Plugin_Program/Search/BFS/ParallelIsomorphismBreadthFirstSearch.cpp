...
extern "C" {
ParallelIsomorphismBreadthFirstSearch* create() {
  return new ParallelIsomorphismBreadthFirstSearch();
}
ParallelIsomorphismBreadthFirstSearch* create_parameter(
...
void ParallelIsomorphismBreadthFirstSearch::search() {
  ...
  cout << left << setw(25) << "Iteration" << setw(25) << "ALLSTATES" << setw(25)
       << "NEWSTATES"
  ...
  while (!newStatesSet.empty()) {
      ...
      for (int i = 1; i <= width + 1; i++) {
        if (bag.vertex_introducible(i)) {
          ...
          State::ptr relabeledNewStatePointer =
              canonicalState(newStatePointer);  // Computes the canonical state
                                                // derived from newStatePointer.
          bool premiseFlag = flags->get("Premise");
          bool satisfiesPremise = false;
          if (premiseFlag) {
            satisfiesPremise =
                conjecture->evaluatePremiseOnState(*relabeledNewStatePointer);
          }
          ...
          if (itX != bagElement.end()) {
            for (auto itPrime = itX; itPrime != bagElement.end(); itPrime++) {
              ...
              State::ptr relabeledNewStatePointer = canonicalState(
                  newStatePointer);  // Computes the canonical state derived
                                     // from newStatePointer.
              bool premiseFlag = flags->get("Premise");
              bool satisfiesPremise = false;
              if (premiseFlag) {
                ...
                if (!allStatesSet.count(relabeledNewStatePointer) and
                  ...
                  if (printStateFlag) {
                    ...
                    cout << "=================================================="
                            "======================"
                         << endl;
                    cout << " Introduce Edge: " << *it << " " << *itPrime
                         << endl;
                    cout << "=================================================="
                            "======================"
                         << endl;
    ...
    for (size_t l = 0; l < newStatesVector.size(); l += per_thread)
      threads.push_back(std::thread(
          visit_range, l, std::min(l + per_thread, newStatesVector.size())));
...
void ParallelIsomorphismBreadthFirstSearch::extractRunTreeNode(
  ...
  if (strstr(wrongSymbol.c_str(), "Leaf")) {
    ...
    if (introducedVertex == 0) {
      cout << "Error: child state is not consistent with the current state"
           << endl;
      exit(20);
    }
    ...
    set_difference(allVertices.begin(), allVertices.end(), setElements.begin(),
                   setElements.end(),
    ...
    do {
      ...
      State::ptr childState1 =
          wrongRunNode->getChildren()[0]->getNodeContent().getState()->relabel(
              compositionMap1);
      // Child 2

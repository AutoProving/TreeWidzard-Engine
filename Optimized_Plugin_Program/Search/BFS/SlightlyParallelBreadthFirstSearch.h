...
class SlightlyParallelBreadthFirstSearch : public SearchStrategy {
  private:
	...
	std::vector<State::ptr> newStatesVector; // This will make it easier to do parallel search 
  public:
    SlightlyParallelBreadthFirstSearch();
	SlightlyParallelBreadthFirstSearch(DynamicKernel *dynamicKernel, Conjecture *conjecture, Flags *flags);
	~SlightlyParallelBreadthFirstSearch(){};
    AbstractTreeDecomposition extractCounterExampleTerm(State::ptr state);
    ////Extract State Tree//////////
    void extractCounterExampleStateTreeNode(State::ptr state, std::shared_ptr<StateTreeNode>);
    StateTree extractCounterExampleStateTree(State::ptr state);
    ///////////////////////////////
    ////Extract Run Tree///////////
    void extractCounterExampleRunNode(State::ptr state, std::shared_ptr<TermNode<RunNodeContent<State::ptr,AbstractTreeDecompositionNodeContent> >> node);

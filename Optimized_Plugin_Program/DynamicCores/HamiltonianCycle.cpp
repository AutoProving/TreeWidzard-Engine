...
bool HamiltonianCycle_Witness::is_equal_implementation(
    const HamiltonianCycle_WitnessPointerConst w) const {
  //*****************************
  //*****************************
...
bool HamiltonianCycle_Witness::is_less_implementation(
    const HamiltonianCycle_WitnessPointerConst w) const {
  //*****************************
  //*****************************
...
shared_ptr<Witness> HamiltonianCycle_Witness::relabel(
  ...
  if (this->degree_0.empty() and this->degree_1.empty() and
    ...
    HamiltonianCycle_WitnessPointer relabeledWitness(
        new HamiltonianCycle_Witness);
    ...
    for (auto d : this->degree_0) {
      if (relabelingMap.count(d)) {
        ...
        cout << "Error: HamiltonianCycle_Witness::relabel " << d
    ...
    for (auto m : matching) {
      if (relabelingMap.count(m.first) and relabelingMap.count(m.second)) {
        ...
        cout << "Error: HamiltonianCycle_Witness::relabel " << m.first << " or "
...
void HamiltonianCycle_Witness::hash(Hasher &h) const {
  h << closed;
  for (unsigned i : degree_0) h << i;
  h << -1u;
  for (unsigned i : degree_1) h << i;
  h << -1u;
  for (unsigned i : degree_2) h << i;
  h << -1u;
  for (auto [i, j] : matching) h << i << j;
}
...
void HamiltonianCycle_DynamicCore::intro_v_implementation(
    unsigned int i, Bag &b, HamiltonianCycle_WitnessPointerConst w,
  ...
  if (w1->degree_0.count(i) == 0 && w1->degree_1.count(i) == 0 &&
      w1->degree_2.count(i) == 0 && !w1->closed) {
...
void HamiltonianCycle_DynamicCore::intro_e_implementation(
  ...
  if (w0->closed) {
    ...
    if (w->matching.at(i) != j) {
      ...
      w1->matching[mi] = mj;
      witnessSet->insert(w1);
    } else if (w->matching.at(i) == j && w->degree_0.size() == 0 &&
               w->degree_1.size() == 2) {
      // They are matching
      HamiltonianCycle_WitnessPointer w1 = createWitness();
      w1->set_equal(*w);
      ...
      witnessSet->insert(std::move(w1));
    }
  }
  //*****************************
  //*****************************
}
void HamiltonianCycle_DynamicCore::forget_v_implementation(
    unsigned int i, Bag &b, HamiltonianCycle_WitnessPointerConst w,
    HamiltonianCycle_WitnessSetPointer witnessSet) {
  //*****************************
  //*****************************
...
void HamiltonianCycle_DynamicCore::join_implementation(
    Bag &b, HamiltonianCycle_WitnessPointerConst w1,
  ...
  for (auto w : {w1, w2}) {
    for (unsigned i : w->degree_0) new_degree[i] += 0;
    for (unsigned i : w->degree_1) new_degree[i] += 1;
    for (unsigned i : w->degree_2) new_degree[i] += 2;
  }

  for (auto [i, d] : new_degree) {
    if (d > 2) return;  // impossible

    if (d == 0) wPrime->degree_0.insert(i);
    if (d == 1) wPrime->degree_1.insert(i);
    if (d == 2) wPrime->degree_2.insert(i);
  }
  ...
  for (auto w : {w1, w2})
    for (auto [i, j] : w->matching)
      if (graph[i].size() == 0 || graph[i][0] != j) graph[i].push_back(j);

  set<unsigned> endpoints;
  for (auto [i, d] : new_degree)
    if (d == 1) endpoints.insert(i);
  ...
  while (endpoints.size()) {
    ...
    while (graph.count(end)) {
      ...
      graph[end].pop_back();
      if (graph[end].size() == 0) graph.erase(end);
  ...
  if (cycle_count >= 2) return;  // impossible
...
HamiltonianCycle_DynamicCore::clean_implementation(
    HamiltonianCycle_WitnessSetPointer witnessSet) {
  return witnessSet;
}

bool HamiltonianCycle_DynamicCore::is_final_witness_implementation(
    HamiltonianCycle_WitnessPointerConst w) {
  //*****************************
  //*****************************
  // Verifies if there is no node with degree 0 or 1, and checks if it's already
  // closed
  if (w->degree_0.size() == 0 && w->degree_1.size() == 0 &&
      w->matching.size() == 0 && w->closed) {
    return true;
  } else {
    return false;
  }
...
void HamiltonianCycle_DynamicCore::copyWitness(
    HamiltonianCycle_WitnessPointer w_input,
...
WitnessSetBasePointer HamiltonianCycle_DynamicCore::intro_v(
  ...
  if (const HamiltonianCycle_Witness *e =
    ...
    cerr << "ERROR: in HamiltonianCycle_DynamicCore::intro_v cast error"
         << endl;
    exit(20);
  }
}
...
WitnessSetBasePointer HamiltonianCycle_DynamicCore::intro_e(
  ...
  if (const HamiltonianCycle_Witness *e =
    ...
    cerr << "ERROR: in HamiltonianCycle_DynamicCore::intro_e cast error"
         << endl;
    exit(20);
  }
}
...
WitnessSetBasePointer HamiltonianCycle_DynamicCore::forget_v(
  ...
  if (const HamiltonianCycle_Witness *e =
    ...
    cerr << "ERROR: in HamiltonianCycle_DynamicCore::forget_v cast error"
         << endl;
    exit(20);
  }
}
...
WitnessSetBasePointer HamiltonianCycle_DynamicCore::join(
  ...
  if (const HamiltonianCycle_Witness *e1 =
    ...
    if (const HamiltonianCycle_Witness *e2 =
            dynamic_cast<const HamiltonianCycle_Witness *>(&witness2)) {
...
bool HamiltonianCycle_DynamicCore::is_final_witness(
  ...
  if (const HamiltonianCycle_Witness *e =
    ...
    cerr
        << "ERROR: in HamiltonianCycle_DynamicCore::is_final_witness cast error"
        << endl;
    exit(20);
  }
}
...
/////////////////////////////////////////////////////////////////////////////////////////////////
extern "C" {
DynamicCore *create() { return new HamiltonianCycle_DynamicCore; }
}

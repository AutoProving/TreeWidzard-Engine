...
std::set<unsigned> DecompositionNode::getVertices(){
...
std::vector<std::shared_ptr<DecompositionNode>> DecompositionNode::getChildren(){
...
void DecompositionNode::setChildren(std::vector<std::shared_ptr<DecompositionNode>> children){
...
void Decomposition::print(){
    ...
    s  = "s td " + std::to_string(label)+ " " + std::to_string(maxBagSize) + " " + std::to_string(vertices.size())+"\n" + s;
    std::cout<<s;
}

std::string Decomposition::printNode(std::shared_ptr<DecompositionNode> node, int &label, int parentLabel,std::list<std::pair<int,int>> &edges){
    label++;
    if(parentLabel != 0)
        edges.push_back(std::make_pair(parentLabel,label));
    int l = label;
    ...
    str+= "\n";
    std::vector<std::shared_ptr<DecompositionNode>> children = node->getChildren();
...
void Decomposition::findMaxBagSizeAndVertices(std::shared_ptr<DecompositionNode> node){

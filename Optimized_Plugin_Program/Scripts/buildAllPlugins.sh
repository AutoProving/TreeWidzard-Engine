...
for eachfile in $(find $PWD/../DynamicCores -type f -name "*.cpp"); do
  ...
  g++ -fsanitize=thread -fPIC -shared $p/../Kernel/WitnessSet.cpp $p/../Kernel/Witness.cpp $p/../Kernel/DynamicCore.cpp $eachfile -o $p/../DynamicPlugins/$name.so &
done
for eachfile in $(find $PWD/../Search -type f -name "*.cpp"); do
  ...
  g++ -fsanitize=thread -fPIC -shared $p/../Kernel/DynamicKernel.cpp $p/../Kernel/DynamicCore.cpp $p/../Conjecture/Conjecture.cpp $p/../Kernel/SearchStrategy.cpp $eachfile -o $p/../SearchPlugins/$name.so &
done

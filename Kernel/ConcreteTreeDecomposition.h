...
public:
    ...
    unsigned readInteger(string::iterator &it);// Reads a string, if the string starts with integers it returns integers else returns -1. Ex "123abc" returns 123 
    ...
    // take a conjecture as an input and check that the concrete tree decomposition satisfies the conjecture or not
    pair<bool,shared_ptr<State>> constructWitnesses(Conjecture &conjecture, shared_ptr<CTDNodeNew> node);

...
// Constructors that takes a node type and bag as input. Used when retrieving a tree decomposition for a counter example.
...
// Constructors that takes a node type, a bag and a vector of children as input. Used when constructing a tree decomposition in a bottom-up fashion.
CTDNodeNew::CTDNodeNew(string nodeType, Bag B, vector<shared_ptr<CTDNodeNew> > children){
    this->set_nodeType(nodeType);
    this->set_B(B);
...
Bag CTDNodeNew::get_B() {
    return this->B;
}

string CTDNodeNew::get_nodeType() {
    return this->nodeType;
}

shared_ptr<CTDNodeNew> CTDNodeNew::get_parent() {
    return this->parent;
}
...
void CTDNodeNew::set_B(Bag B) {
    this->B = B;
}

void CTDNodeNew::set_nodeType(string nodeType) {
...
string CTDNodeNew::printAbstract() {
    return  this->get_nodeType();
}

void CTDNodeNew::printConcrete() {
    this->get_B().print();
}
...
CTDNodeNew CTDNodeNew::introVertex(unsigned i){
        ...
        cout << "ERROR in function CTDNodeNew::introVertex. Number " <<  i << " is not introducible at current bag."<<endl;
        exit(20);
    }
...
CTDNodeNew CTDNodeNew::forgetVertex(unsigned i){
    ...
    if (this->B.vertex_forgettable(i)){
        ...
        cout << "ERROR in function CTDNodeNew::ForgetVertex. Number " <<  i << " is not Forgettable at current bag."<<endl;
        exit(20);
    }
}
...
CTDNodeNew CTDNodeNew::introEdge(unsigned i, unsigned j){
    ...
    if (this->B.edge_introducible(i,j)){
        ...
        vector<shared_ptr<CTDNodeNew> > children;
        children.push_back(this->shared_from_this());
        CTDNodeNew node("IntroEdge_" + to_string(i) + "_" + to_string(j),Bprime,children);
        return node;
    } else {
        cout << "ERROR in function CTDNodeNew::IntroEdge. Number " <<  i <<","<< j << " is not introducible at current bag."<<endl;
        exit(20);
    }
...
CTDNodeNew join(CTDNodeNew left, CTDNodeNew right){
    if (left.get_B().joinable(right.get_B()) ){
        ...
        cout << "ERROR in function friend CTDNodeNew::Join. Two nodes are not joinable."<<endl;
        exit(20);
    }
}
...
void ConcreteTreeDecomposition::traverseNode(CTDNodeNew &node, MultiGraph &G, map<unsigned, unsigned> &colorToVertexMap, unsigned &nVertices,unsigned &nEdges) {
    ...
    if (node.get_nodeType() == "Empty"){
        ...
        set<unsigned> childBagSet = node.get_children()[0]->get_B().get_elements();
        ...
        if(bagSetDifference.size()!=1){
            cout<<"ERROR: ConcreteTreeDecomposition::traverseNode in IntroVertex child's bag and node's bag are not valid"<<endl;
        ...
        if(bagSetDifference.size()!=1){
            cout<<"ERROR: ConcreteTreeDecomposition::traverseNode in ForgetVertex child's bag and node's bag are not valid"<<endl;
            exit(20);
        }
...
string ConcreteTreeDecomposition::printTreeRecursive(CTDNodeNew &node, unsigned &label){ 
    if(node.get_nodeType() == "Join"){
        ...
        label++;
        return s1+s2+to_string(label)+" "+ node.get_nodeType()+"("+to_string(label1)+","+to_string(label2)+") "+node.printCTDNode()+"\n";
        ...
        label++;
        return s +to_string(label)+" "+node.get_nodeType()+"("+to_string(label-1)+") "+node.printCTDNode()+"\n";
    }
}
...
string ConcreteTreeDecomposition::printAbstractRecursive(CTDNodeNew &node, unsigned &label){
...
bool ConcreteTreeDecomposition::readAbstractVertexNumber(string::iterator &it, unsigned  &v) {
    string num;
    string::iterator tempIt = it;
    if(isdigit(*tempIt)){
        num = *tempIt;
...
bool ConcreteTreeDecomposition::readAbstractEdgeNumber(string::iterator &it, pair<unsigned, unsigned> &e) {
    string num;
    unsigned v1=-1;
    unsigned v2=-1;
    string::iterator tempIt = it;
...
shared_ptr<CTDNodeNew>  ConcreteTreeDecomposition::readAbstractExpressionRecursive(string::iterator &it, string::iterator end, vector<shared_ptr<CTDNodeNew> > &v) {
    unsigned label  = 0; 
    ...
    if(isdigit(*it)){
        ...
        if(readToken(it, "IntroVertex_")){
            ...
            if(!b.vertex_introducible(introducedVertex)){
                cout<<"ERROR: it is not "<< introducedVertex << "vertex introducible"<<endl;
                exit(20);
            }else{
            ...
            ctdNode->set_nodeType("IntroEdge_"+to_string(introducedEdge1)+"_"+to_string(introducedEdge2));
            unsigned childLabel = 0 ;
            if(readToken(it,"(")){
                childLabel = readInteger(it);
                if(readToken(it,")")){
            ...
            if(!b.edge_introducible(introducedEdge1,introducedEdge2)){
                cout<<"ERROR:"<<introducedEdge1<<" " <<introducedEdge2<<" are not edge introducible"<<endl;
                exit(20);
            }else{
            ...
            if(!b.vertex_forgettable(forgottenVertex)){
                cout<<"ERROR:" << forgottenVertex << "is not vertex forgetable"<<endl;
            }else{
            ...
            cout<<"ERROR: ConcreteTreeDecomposition::readAbstractExpressionRecursive Token is not matched with defined tokens"<<endl;
            exit(20);
        }
    }
}
...
pair<bool,shared_ptr<State> > ConcreteTreeDecomposition::constructWitnesses(Conjecture &conjecture ,shared_ptr<CTDNodeNew> node){

    // First, We check the type of the node
    if(node->get_nodeType()=="Empty"){
        //if it is an empty, then it is a leaf
        shared_ptr<State> q  = conjecture.kernel->initialState();
        return make_pair(conjecture.evaluateConjectureOnState(*q,conjecture.kernel),q); 
    }else if(strstr(node->get_nodeType().c_str(), "IntroVertex") ){
        pair<bool,shared_ptr<State> > childProcess = constructWitnesses(conjecture, node->get_children()[0]);
        if(!childProcess.first){
            return childProcess;
        }else{
             // find the introduced vertex
             set<unsigned> bagSet = node->get_B().get_elements();
             set<unsigned> childBagSet = childProcess.second->get_bag().get_elements();
             set<unsigned> bagSetDifference;
             set_difference(bagSet.begin(), bagSet.end(),  childBagSet.begin(), childBagSet.end(), std::inserter(bagSetDifference,bagSetDifference.begin()) );
             if(bagSetDifference.size()!=1){
                 cout<<"ERROR: ConcreteTreeDecomposition::constructWitnesses in IntroVertex child's bag and node's bag are not valid"<<endl;
                 exit(20);
            }
            shared_ptr<State> q = conjecture.kernel->intro_v(childProcess.second, *bagSetDifference.begin());
            bool conjectureEvaluationResult = conjecture.evaluateConjectureOnState(*q,conjecture.kernel);
         ...
         if(!childProcess.first){
            return childProcess;
             ...
             set_difference(childBagSet.begin(), childBagSet.end(),bagSet.begin(), bagSet.end(), std::inserter(bagSetDifference,bagSetDifference.begin()) );
             if(bagSetDifference.size()!=1){
                 cout<<"ERROR: ConcreteTreeDecomposition::constructWitnesses in ForgetVertex child's bag and node's bag are not valid"<<endl;
                 exit(20);
            }
            shared_ptr<State> q = conjecture.kernel->forget_v(childProcess.second, *bagSetDifference.begin());
            bool conjectureEvaluationResult = conjecture.evaluateConjectureOnState(*q,conjecture.kernel);
            return make_pair(conjectureEvaluationResult, q);
        }
    }else if(strstr(node->get_nodeType().c_str(), "IntroEdge") ){
         ...
         if(!childProcess.first){
            ...
            pair<unsigned,unsigned> e = childProcess.second->get_bag().get_edge();
            shared_ptr<State> q = conjecture.kernel->intro_e(childProcess.second, e.first, e.second);
            bool conjectureEvaluationResult = conjecture.evaluateConjectureOnState(*q,conjecture.kernel);
            return make_pair(conjectureEvaluationResult, q);
        }
    }else if(strstr(node->get_nodeType().c_str(), "Join") ){
         pair<bool,shared_ptr<State> > childProcess1 = constructWitnesses(conjecture, node->get_children()[0]);
         pair<bool,shared_ptr<State> > childProcess2 = constructWitnesses(conjecture, node->get_children()[1]);
         if(!childProcess1.first){
            return childProcess1;
         }else if(!childProcess2.first){
            return childProcess2;
         }else{
            shared_ptr<State> q = conjecture.kernel->join(childProcess1.second, childProcess2.second);
            bool conjectureEvaluationResult = conjecture.evaluateConjectureOnState(*q, conjecture.kernel);
            return make_pair(conjectureEvaluationResult, q);
         }
    }else{
        cout<<"ERROR in constructWitnesses: The function could not recognize the type of the node"<<endl;
...
bool ConcreteTreeDecomposition::conjectureCheck(Conjecture &conjecture){
    pair<bool, shared_ptr<State>> concreteEvaluation = constructWitnesses(conjecture, root);
    if(!concreteEvaluation.first){
        cout<<" Concrete Tree Decomposition does not satisfy the conjecture"<<endl;
...
shared_ptr<StateTreeNode> ConcreteTreeDecomposition::generateStateTreeNode(shared_ptr<CTDNodeNew> node, shared_ptr<DynamicKernel> kernel) {
    ...
    if(node->get_nodeType()=="Empty"){
        ...
        shared_ptr<StateTreeNode>  childNode = generateStateTreeNode(node->get_children()[0],kernel);
        // find the introduced vertex
        set<unsigned> bagSet = node->get_B().get_elements();
        set<unsigned> childBagSet = node->get_children()[0]->get_B().get_elements();
        set<unsigned> bagSetDifference;
        set_difference(bagSet.begin(), bagSet.end(),  childBagSet.begin(), childBagSet.end(), std::inserter(bagSetDifference,bagSetDifference.begin()) );
        if(bagSetDifference.size()!=1){
            cout<<"ERROR: ConcreteTreeDecomposition::generateStateTreeNode in IntroVertex child's bag and node's bag are not valid"<<endl;
            exit(20);
        }
        shared_ptr<State> q = kernel->intro_v(childNode->get_S(), *bagSetDifference.begin());
        ...
        set_difference(childBagSet.begin(), childBagSet.end(), bagSet.begin(), bagSet.end(), std::inserter(bagSetDifference,bagSetDifference.begin()) );
        if(bagSetDifference.size()!=1){
            cout<<"ERROR: ConcreteTreeDecomposition::generateStateTreeNode in ForgetVertex child's bag and node's bag are not valid"<<endl;
            exit(20);
        }
        shared_ptr<State> q = kernel->forget_v(childNode->get_S(), *bagSetDifference.begin());
        // set the class members of the state tree node
        stateTreeNode->set_nodeType(node->get_nodeType());
        stateTreeNode->set_S(q);
        //create a vector to push the childNode in it
        ...
        if(introducedEdge.first == 0 or introducedEdge.second == 0){
            cout<<"ERROR: ConcreteTreeDecomposition::generateStateTreeNode in IntroEdge the edge isn't valid"<<endl;
            exit(20);
        }
        shared_ptr<State> q = kernel->intro_e(childNode->get_S(), introducedEdge.first, introducedEdge.second);
        // set the class members of the state tree node
        stateTreeNode->set_nodeType(node->get_nodeType());
        stateTreeNode->set_S(q);
        //create a vector to push the childNode in it
        ...
        shared_ptr<StateTreeNode> child1 = generateStateTreeNode(node->get_children()[0], kernel);
        shared_ptr<StateTreeNode> child2 = generateStateTreeNode(node->get_children()[1], kernel);
        ...
        cout<<"ERROR in generateStateTreeNode The function could not  recognize the type of the node"<<endl;
        cout<<"The devastated node is: " <<endl;
        node->printCTDNode();
        exit(20);
    }

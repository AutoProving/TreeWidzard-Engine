...
class StateTreeNode :public std::enable_shared_from_this<StateTreeNode> {
	...
	public:
		...
		StateTreeNode(string nodeType, StatePointer s, vector<shared_ptr<StateTreeNode> > children);
		StateTreeNode(string nodeType, StatePointer s, vector<shared_ptr<StateTreeNode> > children, shared_ptr<DynamicKernel> );
...
class StateTree{
	public:
		...
		StateTreeNodePointer readStateTreeExpressionRecursive(string::iterator &it, string::iterator end, vector<StateTreeNodePointer > &v,DynamicKernel &kernel); // Reads Abstract Tree Decomposition recursively

...
bool CliqueNumberSimpleGraphs_AtLeast_Witness::is_equal_implementation(const CliqueNumberSimpleGraphs_AtLeast_WitnessPointerConst w) const{
    return (this->found == w->found and this->size==w->size and this->partialClique==w->partialClique and this->markedEdges==w->markedEdges);
}
...
//    return false;
    ...
    if (w->found > found)
        return false;
    if(markedEdges<w->markedEdges)
        return true;
    if(w->markedEdges<markedEdges)
        return false;
    if (partialClique < w->partialClique)
        return true;
    if (w->partialClique < partialClique)
        return false;
    return size < w->size;
}
...
Witness& CliqueNumberSimpleGraphs_AtLeast_Witness::set_equal_implementation(CliqueNumberSimpleGraphs_AtLeast_WitnessPointer w){
    ...
    this->markedEdges = w->markedEdges;
    return *this;
}
...
void CliqueNumberSimpleGraphs_AtLeast_DynamicCore::intro_v_implementation(unsigned i, Bag &b, CliqueNumberSimpleGraphs_AtLeast_WitnessPointer w,
     ...
     if(w->found){
         ...
         witness->markedEdges.clear();
         witnessSet->insert(witness);
     }else if(w->size == this->cliqueSize){
         witnessSet->insert(w);
     }else{
...
void CliqueNumberSimpleGraphs_AtLeast_DynamicCore::intro_e_implementation(unsigned i, unsigned j, Bag &b,
    ...
    if (w->found){
       ...
       if(w->partialClique.count(i) and w->partialClique.count(j)){
           ...
           edge.insert(i);
           edge.insert(j);
           if(w->markedEdges.find(edge)==w->markedEdges.end()){
               ...
               witness->markedEdges.insert(edge);
               if(jCounter>cliqueSize or iCounter>cliqueSize){
                   cout<<"jCounter "<<jCounter<< " iCounter "<<iCounter<<endl;
                   exit(20);
               }
...
void CliqueNumberSimpleGraphs_AtLeast_DynamicCore::join_implementation(Bag &b, CliqueNumberSimpleGraphs_AtLeast_WitnessPointer w1,
   ...
   if (w1->found){
       ...
       set<set<unsigned > > intersectionMarketEdge; // intersection should be empty since it is a simple graph
       set_intersection(w1->markedEdges.begin(),w1->markedEdges.end(),w2->markedEdges.begin(),w2->markedEdges.end(),
                        inserter(intersectionMarketEdge,intersectionMarketEdge.begin()));
       ...
       if(intersectionMarketEdge.size()==0 and w1->partialClique.size() == w2->partialClique.size() and b){
           CliqueNumberSimpleGraphs_AtLeast_WitnessPointer witness = createWitness();
           witness->set_equal(*w1);
           ...
           set<set<unsigned> > unionMarkedEdge;
           set_union(w1->markedEdges.begin(),w1->markedEdges.end(),w2->markedEdges.begin(),w2->markedEdges.end(),
                                                        inserter(unionMarkedEdge,unionMarkedEdge.begin()));
           witness->markedEdges = unionMarkedEdge;

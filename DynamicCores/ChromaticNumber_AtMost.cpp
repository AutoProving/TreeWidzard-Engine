...
ChromaticNumber_AtMost_DynamicCore::ChromaticNumber_AtMost_DynamicCore(const vector<char*> &parameters){
    if(parameters.size()!=1){
        cerr<< "ChromaticNumber Core received an invalid parameter list" << endl;
        exit(20);
    }
    this->k = atoi(parameters[0]);;
    createInitialWitnessSet();
    // Initializing attributes
    addAttribute("CoreName","ChromaticNumber");
    addAttribute("CoreType","Bool");

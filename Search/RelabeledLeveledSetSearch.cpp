...
#include "RelabeledLeveledSetSearch.h"

extern "C" {
    RelabeledLeveledSetSearch * create() {
        return new RelabeledLeveledSetSearch();
    }
    RelabeledLeveledSetSearch * create_parameter(DynamicKernel* dynamicKernel, Conjecture* conjecture,Flags* flags) {
        return new RelabeledLeveledSetSearch(dynamicKernel,conjecture,flags);
    }
}

bool check_lessThan_states_RelabeledleveledSetSearch(const State::ptr &lhs,
											const State::ptr &rhs) {
	return *lhs < *rhs;
}

bool check_equality_states_RelabeledleveledSetSearch(const State::ptr &lhs,
											const State::ptr &rhs) {
	return (*lhs == *rhs);
}

RelabeledLeveledSetSearch::RelabeledLeveledSetSearch() {
    addAttribute("SearchName","RelabeledLeveledSetSearch");
}

RelabeledLeveledSetSearch::RelabeledLeveledSetSearch(DynamicKernel *dynamicKernel,
...
unsigned RelabeledLeveledSetSearch::bagSetToNumber(set<unsigned> bagSet,
										  unsigned width) {
	unsigned number = 0;
...
shared_ptr<CTDNodeNew> RelabeledLeveledSetSearch::extractCTDNode(
...
ConcreteTreeDecomposition RelabeledLeveledSetSearch::extractCTDDecomposition(
	unsigned level, State::ptr s,
	vector<vector<set<State::ptr> > > &leveledSetAllStates) {
...
shared_ptr<StateTreeNode> RelabeledLeveledSetSearch::extractStateTreeNode(
	unsigned level, State::ptr s,
...
StateTree RelabeledLeveledSetSearch::extractStateTreeDecomposition(
	unsigned level, State::ptr s,
	vector<vector<set<State::ptr> > > &leveledSetAllStates, bool tree_width) {
	StateTree stateTree;
	stateTree.root =
...
void RelabeledLeveledSetSearch::search() {
	...
	while (newStatesCount) {
			...
			for (auto iteratorNewStates = setNewStates[k].begin();
				...
				for (unsigned i = 1; i <= width + 1; ++i) {
					if ((*iteratorNewStates)->get_bag().vertex_introducible(i)) {
						...
						State::ptr relabeledState = newState->relabel(relabeledMapGenerator(newState->get_bag().get_elements()));
                        unsigned index = bagSetToNumber(relabeledState->get_bag().get_elements(), width);
						setIntermediateStates[index].insert(relabeledState);
						...
						if (flags->get("LoopTime") == 1) {
							for (size_t m = 0;
								 m < relabeledState->numberOfComponents(); m++) {
								if (maxWitnessSetSize[m] < static_cast<unsigned>(relabeledState->getWitnessSet(m)->size())) {
                                    maxWitnessSetSize[m] =relabeledState->getWitnessSet(m)->size();
								}
							}
						}
					}
                ...
                for (size_t i = 1; i < kernel->get_width().get_value() + 2;
                    ...
                    if ((*iteratorNewStates)->get_bag().vertex_forgettable(i)) {
                        ...
                        if (flags->get("LoopTime") == 1) {
                            for (size_t m = 0; m < relabeledState->numberOfComponents(); m++) {
                                if (maxWitnessSetSize[m] <
                                    static_cast<unsigned>(relabeledState->getWitnessSet(m)->size())) {
                                    maxWitnessSetSize[m] = relabeledState->getWitnessSet(m)->size();
                                }
                            }
                        }
				...
				if (kernel->get_width().get_name() == "tree_width") {
					...
					for (auto it = setAllStates[k].begin();
                        ...
                        int index = bagSetToNumber(relabeledState->get_bag().get_elements(), width);
						setIntermediateStates[index].insert(relabeledState);
						// Statistics
						if (flags->get("PrintStates") == 1) {
							cout << "join" << endl;
							...
							for (size_t m = 0;
								...
								if (maxWitnessSetSize[m] <
									static_cast<unsigned>(
                                            relabeledState->getWitnessSet(m)->size())) {
									maxWitnessSetSize[m] =
                                            relabeledState->getWitnessSet(m)->size();
								}
							}
					...
					for (auto itrJoin = setNewStates[k].begin();
                        ...
                        State::ptr relabeledState = newState->relabel(relabeledMapGenerator(newState->get_bag().get_elements()));
                        setIntermediateStates[k].insert(relabeledState);
					}
				}
			}
		}
		...
		for (unsigned bagSetIndex = 0; bagSetIndex < numberBagSets;
			...
			set_difference(setIntermediateStates[bagSetIndex].begin(),
						   ...
						   check_lessThan_states_RelabeledleveledSetSearch);
		}
...
map<unsigned, unsigned> RelabeledLeveledSetSearch::relabeledMapGenerator(set<unsigned int> bagElements) {

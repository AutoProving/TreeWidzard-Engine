...
RelabeledBreadthFirstSearch* create_parameter(DynamicKernel* dynamicKernel,
...
void RelabeledBreadthFirstSearch::search() {
  ...
  while (newStatesSize) {
    ...
    for (int index = 0; index < width + 2; index++) {
      ...
      std::copy(newStatesSet[index].begin(), newStatesSet[index].end(),
                newStatesVector[index].begin());
    ...
    for (int index = 0; index < width + 2; index++) {
      ...
      for (int l = 0; l < newStatesVector[index].size(); l++) {
        ...
        if (bagElements.size() < width + 1) {
          ...
          if (bag.vertex_introducible(i)) {
            ...
            State::ptr relabeledNewStatePointer =
                newStatePointer;  // The relabeling is the identity.
            if (!allStatesSet[bagElements.size() + 1].count(
                    relabeledNewStatePointer) and
                !newStatesSet[bagElements.size() + 1].count(
                    relabeledNewStatePointer)) {
              newStatesSet[bagElements.size() + 1].insert(
                  relabeledNewStatePointer);
              ...
              if (!noBFSDAG) {
                ...
                antecedentStates.push_back(
                    statePointer);  // StatePointer is the only antecedent
                                    // state.
                Transition<State::ptr, AbstractTreeDecompositionNodeContent>
        ...
        if (bag.get_elements().size() > 1) {
          for (auto it = bagElements.begin(); it != bagElements.end(); it++) {
            ...
            if (itX != bagElements.end()) {
              for (auto itPrime = itX; itPrime != bagElements.end();
                   itPrime++) {
                State::ptr newStatePointer =
                    kernel->intro_e(statePointer, *it, *itPrime);
                State::ptr relabeledNewStatePointer =
                    newStatePointer;  // The relabeling is the identity.
                if (!allStatesSet[bagElements.size()].count(
                        relabeledNewStatePointer) and
                    !newStatesSet[bagElements.size()].count(
                        relabeledNewStatePointer)) {
                  newStatesSet[bagElements.size()].insert(
                      relabeledNewStatePointer);
                  State::ptr consequentState = relabeledNewStatePointer;
                  if (!noBFSDAG) {
                    AbstractTreeDecompositionNodeContent transitionContent(
        ...
        if (kernel->get_width().get_name() == "tree_width") {
          ...
          for (auto it = allStatesSet[index].begin();
               it != allStatesSet[index].end(); it++) {
            // Testing possibility of joining with each permuted version of
            // "it".
            map<unsigned, unsigned> m = identityMap(bagElements);
            do {
              State::ptr permutedState = (*it)->relabel(m);
              State::ptr newStatePointer =
                  kernel->join(statePointer, permutedState);
              State::ptr relabeledNewStatePointer =
                  newStatePointer;  // The identity relabeling.
              // for each relabeling of the bag from it, you do the join between
              // the statePointer and the relabeled version of it.
    ...
    for (int index = 0; index < width + 2; index++) {
      for (auto it = newStatesSet[index].begin();
           it != newStatesSet[index].end(); it++) {
        if (!conjecture->evaluateConjectureOnState(**it)) {
          ...
          Term<AbstractTreeDecompositionNodeContent>* term =
              new AbstractTreeDecomposition;
          *term = runTree.convertRunToTerm(runTree);
          AbstractTreeDecomposition* atd =
              static_cast<AbstractTreeDecomposition*>(term);
          atd->printTermNodes();
      ...
      set_union(allStatesSet[index].begin(), allStatesSet[index].end(),
                newStatesSet[index].begin(), newStatesSet[index].end(),
    ...
    if (flags->get("LoopTime") == 1) {
      std::cout << "AllState:" << allStatesSize
                << " new State: " << newStatesSize << std::endl;
      std::cout << std::endl
...
map<unsigned, unsigned> RelabeledBreadthFirstSearch::relabeledMapGenerator(
...
map<unsigned int, unsigned int> RelabeledBreadthFirstSearch::identityMap(
    unsigned int k) {
...
bool RelabeledBreadthFirstSearch::nextPermutation(
...
void RelabeledBreadthFirstSearch::extractRunTreeNode(
  ...
  if (strstr(wrongSymbol.c_str(), "Leaf")) {
    if (m.empty()) {
      ...
      std::cout << "Error: In Empty node, map is not valid. " << std::endl;
      exit(20);
    }
    ...
    vector<int> numbers = abs.extractIntegerWords(
        wrongSymbol);  // Extract the integers inside the string.
    if (numbers.size() == 1) {
      abs.setSymbol("IntroVertex_" + to_string(m[numbers[0]]));
    } else {
      std::cout << "Error: in IntroVertex numbers is not valid." << std::endl;
      exit(20);
    }
    std::cout << "new symbol -----> " << abs.getSymbol() << std::endl;
    RunNodeContent<State::ptr, AbstractTreeDecompositionNodeContent>
        correctedRunNodeContent(abs, correctedState);
    correctedRunNode->setNodeContent(correctedRunNodeContent);
    m.erase(numbers[0]);
    ...
    vector<int> numbers = abs.extractIntegerWords(
        wrongSymbol);  // Extract the integers inside the string.
    if (numbers.size() == 2) {
      abs.setSymbol("IntroEdge_" + to_string(m[numbers[0]]) + "_" +
                    to_string(m[numbers[1]]));
    } else {
      std::cout << "Error: in IntroEdge numbers is not valid." << std::endl;
      exit(20);
    }
    std::cout << "new symbol -----> " << abs.getSymbol() << std::endl;
    std::cout << "modified map===============" << std::endl;
    ...
    // Child 2
    map<unsigned, unsigned> m2 =
        identityMap(correctedState->get_bag().get_elements());
    // We need to discover a map that is compatible with the join.
    State::ptr joinState;
    bool flag = false;
    do {
      map<unsigned, unsigned>
          compositionMap;  // Changes the domain of the child
      for (auto item : m) {
        compositionMap.insert(make_pair(item.first, m2[item.second]));
      }
      State::ptr state =
          wrongRunNode->getChildren()[1]->getNodeContent().getState()->relabel(
              compositionMap);
    ...
    if (flag) {
      shared_ptr<TermNode<
          ...
          child2(new TermNode<RunNodeContent<
                     State::ptr, AbstractTreeDecompositionNodeContent>>);
      child2->setParent(correctedRunNode);
      children.push_back(child2);
      extractRunTreeNode(wrongRunNode->getChildren()[0], child2, m2);
    } else {
      std::cout << "Error: join is not valid.";
      exit(20);
    }
    correctedRunNode->setChildren(children);
  } else {
    std::cout << "Error: Node Type is not valid." << std::endl;
    exit(20);
  }
}
...
RelabeledBreadthFirstSearch::extractRunTree(State::ptr state) {
  RunTree<State::ptr, AbstractTreeDecompositionNodeContent> wrongRunTree =

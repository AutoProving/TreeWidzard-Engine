...
extern "C" {
    ...
    LeveledSetSearchPremise * create_parameter(DynamicKernel* dynamicKernel, Conjecture* conjecture,Flags* flags) {
        return new LeveledSetSearchPremise(dynamicKernel,conjecture,flags);
    }
}
bool check_lessThan_states_leveledSetSearchPremise (const State::ptr &lhs, const State::ptr &rhs) {
    return *lhs < *rhs;
}

bool check_equality_states_leveledSetSearchPremise (const State::ptr &lhs, const State::ptr &rhs) {
    return (*lhs == *rhs);
}

LeveledSetSearchPremise::LeveledSetSearchPremise() {
    addAttribute("SearchName","LeveledSetSearchPremise");
}

LeveledSetSearchPremise::LeveledSetSearchPremise(DynamicKernel *dynamicKernel, Conjecture *conjecture, Flags *flags) : SearchStrategy(dynamicKernel, conjecture, flags) {
    this->kernel = kernel;
    this->conjecture = conjecture;
    this->flags = flags;
}

unsigned LeveledSetSearchPremise::bagSetToNumber(set<unsigned> bagSet,unsigned width){
    unsigned number = 0;
...
void LeveledSetSearchPremise::search(){
    ...
    while(newStatesCount){
        ...
        for (unsigned bagSetIndex = 0; bagSetIndex < numberBagSets; bagSetIndex++){
            ...
            set_difference(setIntermediateStates[bagSetIndex].begin(),setIntermediateStates[bagSetIndex].end(),setAllStates[bagSetIndex].begin(),setAllStates[bagSetIndex].end(),inserter(setNewStates[bagSetIndex], setNewStates[bagSetIndex].begin()), check_lessThan_states_leveledSetSearchPremise);
        }
        ...
        for (unsigned bagSetIndex = 0; bagSetIndex < numberBagSets; bagSetIndex++){
            set<State::ptr> temp;
            set_union(setAllStates[bagSetIndex].begin(),setAllStates[bagSetIndex].end(),setNewStates[bagSetIndex].begin(),setNewStates[bagSetIndex].end(),inserter(temp,temp.begin()),check_lessThan_states_leveledSetSearchPremise);

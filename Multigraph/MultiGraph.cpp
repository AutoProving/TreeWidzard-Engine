...
void MultiGraph::setIncidenceMap(const multimap<unsigned,unsigned> &incidenceMap) {
    MultiGraph::incidenceMap = incidenceMap;
}

const set<unsigned> &MultiGraph::getVertices() const {
    return vertices;
}

const set<unsigned> &MultiGraph::getEdges() const {
    return edges;
}
...
void MultiGraph::addVertex(unsigned vertex) {
    this->vertices.insert(vertex);
}

void MultiGraph::addEdgeLabel(unsigned edge) {
...
set<unsigned> MultiGraph::edgesVertex(unsigned i) {
    set<unsigned> edges;
    for(auto it = this->incidenceMap.begin(); it != this->incidenceMap.end() ; it++){
        if(it->second==i){
...
set<unsigned> MultiGraph::edgesBetweenVertices(unsigned i, unsigned j) {
    ...
    set_intersection(edgesI.begin(), edgesI.end(), edgesJ.begin(), edgesJ.end(), inserter(sharedEdges,sharedEdges.begin()));
    return sharedEdges;
}
...
void MultiGraph::printGraph() {
    cout<<"vertices:"<<endl;
    for(set<unsigned>::iterator it = this->vertices.begin(); it != this->vertices.end() ; ++it){
        cout<<*it<<endl;
    }
    cout<<"Edges"<<endl;
        ...
        if(str == "vertices"){
            ...
            if (edgeC == 0) {
                if(incidenceMap.count(stoi(str))!=0) {
                    cout << "ERROR: in MultiGraph::readFromFile there are two edges with same name" << endl;
                    exit(20);
                }
                ...
                cout<<"ERROR: in MultiGraph::readFromFile format did not matched"<<endl;
                exit(20);
            }
        }else{
            cout<<"ERROR: in MultiGraph::readFromFile format did not matched"<<endl;
...
bool MultiGraph::convertToGML(){
    ...
    for(auto itr  = edges.begin() ; itr != edges.end() ; itr++){
        ...
        if(std::distance(values.first,  values.second) == 2){
            ...
            cout<<"ERROR: MultiGraph::convertToGML multigraph is not valid"<<endl;
            return false;
        }

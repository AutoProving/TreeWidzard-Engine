...
class RunNodeContent : public TermNodeContentType {
  ...
  public:
	...
	RunNodeContent(TermNodeContent runNodeContent, StateType state)
		: runNodeContent(runNodeContent), state(state){};

	RunNodeContent(
		const RunNodeContent<StateType, TermNodeContent> &runNodeContent);

	TermNodeContent getRunNodeContent() const { return runNodeContent; }
	...
	void setState(StateType state) { this->state = state; }

	RunNodeContent<StateType, TermNodeContent> &operator=(
		const RunNodeContent<StateType, TermNodeContent> &other);

	bool operator==(const RunNodeContent &rhs) const {
		return runNodeContent == rhs.runNodeContent && state == rhs.state;
	}

	bool operator!=(const RunNodeContent &rhs) const { return !(rhs == *this); }

	bool operator<(const RunNodeContent &rhs) const {
		if (runNodeContent < rhs.runNodeContent) return true;
		if (rhs.runNodeContent < runNodeContent) return false;
	...
	std::string nodeInformation() override {
		std::string info;
		info =
			runNodeContent.nodeInformation() + " " + state.stateInformation();
		info +=
			"---------------------------------------------------------------\n";
		info +=
			"---------------------------------------------------------------\n";
...
template <class StateType, class TermNodeContent>
RunNodeContent<StateType, TermNodeContent>::RunNodeContent(
	const RunNodeContent<StateType, TermNodeContent> &other) {
...
// TODO: what's the point of this unused argument?
template <class StateType, class TermNodeContent>
Term<TermNodeContent> RunTree<StateType, TermNodeContent>::convertRunToTerm(
	RunTree<StateType, TermNodeContent>) {
	Term<TermNodeContent> term;
	std::shared_ptr<TermNode<TermNodeContent>> termRoot(
...
template <class StateType, class TermNodeContent>
void RunTree<StateType, TermNodeContent>::convertRunNodeToTermNode(
	std::shared_ptr<TermNode<RunNodeContent<StateType, TermNodeContent>>>
		runNode,
	std::shared_ptr<TermNode<TermNodeContent>> termNode) {
	...
	for (auto item : runNode->getChildren()) {
		std::shared_ptr<TermNode<TermNodeContent>> termChild(
			new TermNode<TermNodeContent>);

...
bool ConcreteNode::operator==(const ConcreteNode &rhs) const {
	return bag == rhs.bag && symbol == rhs.symbol;
}
bool ConcreteNode::operator!=(const ConcreteNode &rhs) const {
	return !(rhs == *this);
}
bool ConcreteNode::operator<(const ConcreteNode &rhs) const {
	if (bag < rhs.bag) return true;
	if (rhs.bag < bag) return false;
...
MultiGraph ConcreteTreeDecomposition::extractMultiGraph() {
	...
	std::set<unsigned> elements =
		this->getRoot()->getNodeContent().getBag().get_elements();
	std::set<unsigned>::iterator it = elements.begin();
	for (size_t i = 1;
		 i <= this->getRoot()->getNodeContent().getBag().get_elements().size();
		 ++i) {
...
void ConcreteTreeDecomposition::traverseNode(
	TermNode<ConcreteNode> &node, MultiGraph &G,
	std::map<unsigned int, unsigned int> &colorToVertexMap,
	unsigned int &nVertices, unsigned int &nEdges) {
	std::map<unsigned, unsigned> colorToVertexMapCopy = colorToVertexMap;
	if (node.getNodeContent().getSymbol() == "Leaf") {
		// do nothing
	} else if (strstr(node.getNodeContent().getSymbol().c_str(),
					  "IntroVertex")) {
		/////////////// Finding the introduced vertex ///////////
		std::set<unsigned> bagSet =
			node.getNodeContent().getBag().get_elements();
...
Decomposition ConcreteTreeDecomposition::extractDecomposition() {
	...
	buildDecompositionBags(root, *(this->getRoot()), colorToVertexMap,
						   nVertices);
	decomposition.setRoot(root);
	return decomposition;
}

void ConcreteTreeDecomposition::buildDecompositionBags(
	std::shared_ptr<DecompositionNode> node, TermNode<ConcreteNode> &cNode,
	std::map<unsigned, unsigned> &colorToVertexMap, unsigned &nVertices) {
	std::map<unsigned, unsigned> colorToVertexMapCopy = colorToVertexMap;
	if (cNode.getNodeContent().getSymbol() == "Leaf") {
		// do nothing
	} else if (strstr(cNode.getNodeContent().getSymbol().c_str(),
					  "IntroVertex")) {
		/////////////// Finding the introduced vertex ///////////
		std::set<unsigned> bagSet =
			cNode.getNodeContent().getBag().get_elements();
		std::set<unsigned> childBagSet =
		...
		if (bagSetDifference.size() != 1) {
			std::cout << "ERROR: "
						 "ConcreteTreeDecomposition::buildDecompositionBags in "
						 "IntroVertex child's bag and node's bag are not valid"
					  << std::endl;
			exit(20);
		}
		...
		buildDecompositionBags(
			node, *(cNode.getChildren()[0]), colorToVertexMapCopy,
		...
		node->addChild(nNode);
		buildDecompositionBags(
			nNode, *(cNode.getChildren()[0]), colorToVertexMapCopy,
			nVertices); // Nothing happens. Just process the next bag.
	} else if (strstr(cNode.getNodeContent().getSymbol().c_str(),
					  "IntroEdge")) {
		buildDecompositionBags(node, *(cNode.getChildren()[0]),
							   colorToVertexMapCopy, nVertices);
	} else if (cNode.getNodeContent().getSymbol() == "Join") {
		buildDecompositionBags(node, *(cNode.getChildren()[0]),
							   colorToVertexMapCopy, nVertices);
		buildDecompositionBags(node, *(cNode.getChildren()[1]),
							   colorToVertexMapCopy, nVertices);
	}
}
...
void ConcreteTreeDecomposition::writeToFile(std::string fileName) {
	// fileName =
	// "CounterExample_ConcreteTreeDecomposition_"+concrete_fs::path(fileName).filename().string();
...
ConcreteTreeDecomposition::constructWitnesses(
	...
	if (node->getNodeContent().getSymbol() == "Leaf") {
		...
		std::shared_ptr<TermNode<RunNodeContent<State::ptr, ConcreteNode>>>
			runNode(new TermNode<RunNodeContent<State::ptr, ConcreteNode>>);
		std::shared_ptr<RunNodeContent<State::ptr, ConcreteNode>>
			runNodeContent(new RunNodeContent<State::ptr, ConcreteNode>);
		...
		if (bagSetDifference.size() != 1) {
			std::cout << "ERROR: ConcreteTreeDecomposition::constructWitnesses "
					  ...
					  << ":child's bag and node's bag are not valid"
					  << std::endl;
			std::cout << "node's bag: ";
		...
		State::ptr q = conjecture.getKernel()->forget_v(
			childState, *bagSetDifference.begin());
		if (flags.get("PrintStates")) {
			q->print();
		}
		...
		std::pair<unsigned, unsigned> e =
			node->getNodeContent().getBag().get_edge();
		State::ptr q =
			conjecture.getKernel()->intro_e(childState, e.first, e.second);
		conjecture.evaluateConjectureOnState(*q);
		// TODO: Why is this a float?
		if (bool(flags.get("PrintStates"))) {
			q->print();
		}
		std::shared_ptr<TermNode<RunNodeContent<State::ptr, ConcreteNode>>>
			runNode(new TermNode<RunNodeContent<State::ptr, ConcreteNode>>);
		...
		std::cout
			<< "ERROR in constructWitnesses: The function could not recognize "
...
bool ConcreteTreeDecomposition::conjectureCheck(Conjecture &conjecture,
	...
	//    std::cout <<
	//    conjecture.evaluateConjectureOnState(*runNode->getNodeContent().getState())

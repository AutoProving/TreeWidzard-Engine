class AbstractTreeDecompositionSymbol{
	...
	int symbolType(); // returns "0 if Leaf, 1 if IntroVertex, 2 if  ForgetVertex, 3 if IntroEdge or 4 if Join"
	vector<int> numbersSymbol(); // returns vector "[i]" if symbol="IntroVertex_i or ForgetVertex_i", Returns vector "[i,j]" if symbol="IntroEdge_i_j", Returns empty vector if symbol = "Join"  
	bool operator== (const AbstractTreeDecompositionSymbol &rhs) const {return this->symbol == rhs}; 
	void setSymbol(strint symbol); 
}
...
private:
	SymbolType symbol; 
	vector<shared_ptr<TermNode> > children; 
...
public:
	...
	void setChildren(shared_ptr<TermNode> children)
	void setParent(shared_ptr<TermNode> parent)
	//get functions
	symbol getSymbol();
	shared_ptr getParent();
	shared_ptr getChildren();
	shared_ptr getChild(int i); // returns a pointer to the i-th child
};
...
public: 
	...
	void setSymbol(SymbolType symbol); 
	void setConsequentStates(vector<StateType> antecedentStates)
}; 
...
public:
	...
	bool findSymbol(AlphabetType symbol)

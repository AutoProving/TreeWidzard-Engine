...
bool HasMultipleEdges_Witness::is_equal_implementation(
	const HasMultipleEdges_WitnessPointerConst w) const {
	return found == w->found and edgeSet == w->edgeSet;
}

bool HasMultipleEdges_Witness::is_less_implementation(
	const HasMultipleEdges_WitnessPointerConst w) const {
	if (found < w->found) {
		return true;
...
Witness &HasMultipleEdges_Witness::set_equal_implementation(
...
shared_ptr<Witness> HasMultipleEdges_Witness::relabel(
	...
	if (this->found) {
		...
		for (auto p : this->edgeSet) {
			...
			if (it != relabelingMap.end() and itr != relabelingMap.end()) {
				...
				cout << "Error: HasMultipleEdges_Witness::relabel " << p.first
					 << " or " << p.second << " is not in the map" << endl;
				print();
				cout << "\nmap" << endl;
...
void HasMultipleEdges_Witness::print() const {
	...
	cout << "{";
	for (set<pair<unsigned, unsigned>>::iterator it = wEdgeSet.begin();
		 it != wEdgeSet.end(); ++it) {
...
HasMultipleEdges_Witness::HasMultipleEdges_Witness() { found = false; }
...
HasMultipleEdges_DynamicCore::HasMultipleEdges_DynamicCore(
	...
	if (parameter.size())
		throw std::runtime_error(
			"HasMultipleEdges does not take any arguments.");
}
...
void HasMultipleEdges_DynamicCore::intro_v_implementation(
	unsigned int, const Bag &, HasMultipleEdges_WitnessPointerConst w,
...
void HasMultipleEdges_DynamicCore::intro_e_implementation(
...
void HasMultipleEdges_DynamicCore::forget_v_implementation(
	unsigned int i, const Bag &b, HasMultipleEdges_WitnessPointerConst w,
	HasMultipleEdges_WitnessSetPointer witnessSet) {
	HasMultipleEdges_WitnessPointer witness = createWitness();
	...
	if (w->found) {
		...
		for (set<unsigned>::iterator it = tempBag.begin(); it != tempBag.end();
			 it++) {
...
void HasMultipleEdges_DynamicCore::join_implementation(
	const Bag &, HasMultipleEdges_WitnessPointerConst w1,
	...
	if (w1->found) {
		...
		set_intersection(
			wOneSet.begin(), wOneSet.end(), wTwoSet.begin(), wTwoSet.end(),
		...
		if (intersection_set.size() > 0) {
			...
			set_union(wOneSet.begin(), wOneSet.end(), wTwoSet.begin(),
					  wTwoSet.end(),
...
bool HasMultipleEdges_DynamicCore::is_final_witness_implementation(
	HasMultipleEdges_WitnessPointerConst w) {
	return w->found;
}

HasMultipleEdges_WitnessSetPointer
HasMultipleEdges_DynamicCore::clean_implementation(
	HasMultipleEdges_WitnessSetPointer witnessSet) {
	return witnessSet;
}
...
bool HasMultipleEdges_Witness::is_equal(const Witness &rhs) const {
	if (HasMultipleEdges_Witness const *e =
			dynamic_cast<HasMultipleEdges_Witness const *>(&rhs)) {
		HasMultipleEdges_WitnessPointerConst w = e->shared_from_this();
		return is_equal_implementation(w);
	} else {
		cerr << "ERROR: in HasMultipleEdges_Witness::is_equal cast error"
			 << endl;
		exit(20);
	}
}
...
bool HasMultipleEdges_Witness::is_less(const Witness &rhs) const {
	if (HasMultipleEdges_Witness const *e =
		...
		cerr << "ERROR: in HasMultipleEdges_Witness::is_less cast error"
			 << endl;
		exit(20);
	}
}
...
Witness &HasMultipleEdges_Witness::set_equal(const Witness &witness) {
	if (const HasMultipleEdges_Witness *e =
			dynamic_cast<const HasMultipleEdges_Witness *>(&witness)) {
		HasMultipleEdges_WitnessPointerConst w = e->shared_from_this();
		return set_equal_implementation(w);
	} else {
		cerr << "ERROR: in HasMultipleEdges_Witness::operator= cast error"
			 << endl;
		exit(20);
	}
}
...
shared_ptr<WitnessSet> HasMultipleEdges_WitnessSet::createEmptyWitnessSet() {
	HasMultipleEdges_WitnessSetPointer witnessSet(
		new HasMultipleEdges_WitnessSet);
	return witnessSet;
}
...
void HasMultipleEdges_DynamicCore::copyWitness(
	HasMultipleEdges_WitnessPointer w_input,
...
WitnessSetPointer HasMultipleEdges_DynamicCore::intro_v(
	...
	if (const HasMultipleEdges_Witness *e =
		...
		cerr << "ERROR: in HasMultipleEdges_DynamicCore::intro_v cast error"
			 << endl;
		exit(20);
	}
}

WitnessSetPointer HasMultipleEdges_DynamicCore::intro_e(
	...
	if (const HasMultipleEdges_Witness *e =
		...
		cerr << "ERROR: in HasMultipleEdges_DynamicCore::intro_e cast error"
			 << endl;
		exit(20);
	}
}

WitnessSetPointer HasMultipleEdges_DynamicCore::forget_v(
	unsigned i, const Bag &b, const Witness &witness) {
	if (const HasMultipleEdges_Witness *e =
			dynamic_cast<const HasMultipleEdges_Witness *>(&witness)) {
		HasMultipleEdges_WitnessPointerConst w = e->shared_from_this();
		...
		cerr << "ERROR: in HasMultipleEdges_DynamicCore::forget_v cast error"
			 << endl;
		exit(20);
	}
}

WitnessSetPointer HasMultipleEdges_DynamicCore::join(const Bag &b,
													 const Witness &witness1,
													 const Witness &witness2) {
	if (const HasMultipleEdges_Witness *e1 =
			dynamic_cast<const HasMultipleEdges_Witness *>(&witness1)) {
		if (const HasMultipleEdges_Witness *e2 =
				dynamic_cast<const HasMultipleEdges_Witness *>(&witness2)) {
			...
			cerr << "ERROR: in HasMultipleEdges_DynamicCore::join cast error"
				 << endl;
			exit(20);
		}
	} else {
...
WitnessSetPointer HasMultipleEdges_DynamicCore::clean(
	WitnessSetPointer witnessSet) {
	if (HasMultipleEdges_WitnessSetPointer e =
			dynamic_pointer_cast<HasMultipleEdges_WitnessSet>(witnessSet)) {
		return clean_implementation(e);
	} else {
		cerr << "ERROR: in HasMultipleEdges_DynamicCore::clean cast error"
			 << endl;
		exit(20);
	}
}

bool HasMultipleEdges_DynamicCore::is_final_witness(const Bag &,
	...
	if (const HasMultipleEdges_Witness *e =
		...
		cerr << "ERROR: in HasMultipleEdges_DynamicCore::is_final_witness cast "
				"error"
			 << endl;
		exit(20);
	}
...
extern "C" {
DynamicCore *create(const parameterType &parameter) {
	return new HasMultipleEdges_DynamicCore(parameter);
}
std::map<std::string, std::string> *metadata() {
	return new std::map<std::string, std::string>(
		HasMultipleEdges_DynamicCore().getAttributes());
}
}

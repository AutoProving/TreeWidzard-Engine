...
bool CliqueNumberSimpleGraphs_AtLeast_Witness::is_equal_implementation(
	const CliqueNumberSimpleGraphs_AtLeast_WitnessPointerConst w) const {
	return (this->found == w->found and this->size == w->size and
			this->partialClique == w->partialClique);
}

bool CliqueNumberSimpleGraphs_AtLeast_Witness::is_less_implementation(
	const CliqueNumberSimpleGraphs_AtLeast_WitnessPointerConst w) const {
	if (this->found > w->found) return true;
	if (this->found == w->found and this->size < w->size) return true;
	if (this->found == w->found and this->size == w->size)
		return this->partialClique < w->partialClique;
	return false;
}

Witness &CliqueNumberSimpleGraphs_AtLeast_Witness::set_equal_implementation(
	CliqueNumberSimpleGraphs_AtLeast_WitnessPointerConst w) {
	this->found = w->found;
	this->size = w->size;
	this->partialClique = w->partialClique;
	return *this;
...
shared_ptr<Witness> CliqueNumberSimpleGraphs_AtLeast_Witness::relabel(
	const map<unsigned int, unsigned int> &relabelingMap) const {
	// Here we can not call createWitness() because it is not accessible in this
	// function.
	CliqueNumberSimpleGraphs_AtLeast_WitnessPointer relabeledWitness(
		new CliqueNumberSimpleGraphs_AtLeast_Witness);
	...
	for (auto p : this->partialClique) {
		...
		if (it != relabelingMap.end()) {
			relabeledWitness->partialClique.insert(
				make_pair(it->second, p.second));
		} else {
			cout << "Error: CliqueNumberSimpleGraphs_AtLeast_Witness::relabel "
				 << p.first << "in the map" << endl;
			print();
...
void CliqueNumberSimpleGraphs_AtLeast_DynamicCore::
	createInitialWitnessSet_implementation(WitnessSet &witnessSet) {
...
void CliqueNumberSimpleGraphs_AtLeast_DynamicCore::intro_v_implementation(
	unsigned i, const Bag &,
	CliqueNumberSimpleGraphs_AtLeast_WitnessPointerConst w,
	...
	if (w->found or w->size == this->cliqueSize) {
		...
		if (w->size == w->partialClique.size()) {
			CliqueNumberSimpleGraphs_AtLeast_WitnessPointer witnessTwo =
				createWitness();
...
void CliqueNumberSimpleGraphs_AtLeast_DynamicCore::intro_e_implementation(
	unsigned i, unsigned j, const Bag &,
	CliqueNumberSimpleGraphs_AtLeast_WitnessPointerConst w,
	CliqueNumberSimpleGraphs_AtLeast_WitnessSetPointer witnessSet) {
	if (w->found) {
		CliqueNumberSimpleGraphs_AtLeast_WitnessPointer witness =
			createWitness();
		witness->set_equal(*w);
		witnessSet->insert(witness);
	} else {
...
void CliqueNumberSimpleGraphs_AtLeast_DynamicCore::forget_v_implementation(
	unsigned i, const Bag &,
	CliqueNumberSimpleGraphs_AtLeast_WitnessPointerConst w,
	CliqueNumberSimpleGraphs_AtLeast_WitnessSetPointer witnessSet) {
	CliqueNumberSimpleGraphs_AtLeast_WitnessPointer witness = createWitness();
...
void CliqueNumberSimpleGraphs_AtLeast_DynamicCore::join_implementation(
	const Bag &, CliqueNumberSimpleGraphs_AtLeast_WitnessPointerConst w1,
	CliqueNumberSimpleGraphs_AtLeast_WitnessPointerConst w2,
	CliqueNumberSimpleGraphs_AtLeast_WitnessSetPointer witnessSet) {
	CliqueNumberSimpleGraphs_AtLeast_WitnessPointer witness = createWitness();
	if (w1->found) {
...
CliqueNumberSimpleGraphs_AtLeast_WitnessSetPointer
CliqueNumberSimpleGraphs_AtLeast_DynamicCore::clean_implementation(
	CliqueNumberSimpleGraphs_AtLeast_WitnessSetPointer witnessSet) {
	for (auto witness : (*witnessSet)) {
		if (CliqueNumberSimpleGraphs_AtLeast_WitnessPointer w =
				dynamic_pointer_cast<CliqueNumberSimpleGraphs_AtLeast_Witness>(
					witness)) {
			if (w->found) {
				CliqueNumberSimpleGraphs_AtLeast_WitnessSetPointer
					newWitnessSet(
						new CliqueNumberSimpleGraphs_AtLeast_WitnessSet);
				newWitnessSet->insert(w);
				return newWitnessSet;
			}
		} else {
			cerr << "ERROR: in "
					"CliqueNumberSimpleGraphs_AtLeast_DynamicCore::clean_"
					"implementation cast error"
				 << endl;
			exit(20);
		}
	}
...
bool CliqueNumberSimpleGraphs_AtLeast_DynamicCore::
	is_final_witness_implementation(
		CliqueNumberSimpleGraphs_AtLeast_WitnessPointerConst w) {
	return w->found;
}

bool CliqueNumberSimpleGraphs_AtLeast_DynamicCore::isCompleteClique(
	CliqueNumberSimpleGraphs_AtLeast_WitnessPointerConst w) {
...
//////////////////////////////////////////////////////////////////////////////////////////
bool CliqueNumberSimpleGraphs_AtLeast_Witness::is_equal(
	const Witness &rhs) const {
	if (CliqueNumberSimpleGraphs_AtLeast_Witness const *e =
			dynamic_cast<CliqueNumberSimpleGraphs_AtLeast_Witness const *>(
				&rhs)) {
		CliqueNumberSimpleGraphs_AtLeast_WitnessPointerConst w =
			e->shared_from_this();
		return is_equal_implementation(w);
	} else {
		cerr << "ERROR: in CliqueNumberSimpleGraphs_AtLeast_Witness::is_equal "
				"cast error"
			 << endl;
		exit(20);
	}
...
bool CliqueNumberSimpleGraphs_AtLeast_Witness::is_less(
	const Witness &rhs) const {
	if (CliqueNumberSimpleGraphs_AtLeast_Witness const *e =
			dynamic_cast<CliqueNumberSimpleGraphs_AtLeast_Witness const *>(
				&rhs)) {
		...
		cerr << "ERROR: in CliqueNumberSimpleGraphs_AtLeast_Witness::is_less "
				"cast error"
			 << endl;
		exit(20);
	}
...
Witness &CliqueNumberSimpleGraphs_AtLeast_Witness::set_equal(
	const Witness &witness) {
	if (const CliqueNumberSimpleGraphs_AtLeast_Witness *e =
			dynamic_cast<const CliqueNumberSimpleGraphs_AtLeast_Witness *>(
				&witness)) {
		CliqueNumberSimpleGraphs_AtLeast_WitnessPointerConst w =
			e->shared_from_this();
		return set_equal_implementation(w);
	} else {
		cerr << "ERROR: in CliqueNumberSimpleGraphs_AtLeast_Witness::operator= "
				"cast error"
			 << endl;
		exit(20);
	}
...
CliqueNumberSimpleGraphs_AtLeast_WitnessSet::createEmptyWitnessSet() {
	CliqueNumberSimpleGraphs_AtLeast_WitnessSetPointer witnessSet(
		new CliqueNumberSimpleGraphs_AtLeast_WitnessSet);
	return witnessSet;
}

CliqueNumberSimpleGraphs_AtLeast_WitnessPointer
CliqueNumberSimpleGraphs_AtLeast_DynamicCore::createWitness() {
	CliqueNumberSimpleGraphs_AtLeast_WitnessPointer w(
		new CliqueNumberSimpleGraphs_AtLeast_Witness);
	return w;
}

void CliqueNumberSimpleGraphs_AtLeast_DynamicCore::copyWitness(
	CliqueNumberSimpleGraphs_AtLeast_WitnessPointer w_input,
...
WitnessSetPointer CliqueNumberSimpleGraphs_AtLeast_DynamicCore::intro_v(
	...
	if (const CliqueNumberSimpleGraphs_AtLeast_Witness *e =
		...
		cerr << "ERROR: in "
				"CliqueNumberSimpleGraphs_AtLeast_DynamicCore::intro_v cast "
				"error"
			 << endl;
		exit(20);
	}
...
WitnessSetPointer CliqueNumberSimpleGraphs_AtLeast_DynamicCore::intro_e(
	...
	if (const CliqueNumberSimpleGraphs_AtLeast_Witness *e =
		...
		cerr << "ERROR: in "
				"CliqueNumberSimpleGraphs_AtLeast_DynamicCore::intro_e cast "
				"error"
			 << endl;
		exit(20);
	}
...
WitnessSetPointer CliqueNumberSimpleGraphs_AtLeast_DynamicCore::forget_v(
	unsigned i, const Bag &b, const Witness &witness) {
	if (const CliqueNumberSimpleGraphs_AtLeast_Witness *e =
			dynamic_cast<const CliqueNumberSimpleGraphs_AtLeast_Witness *>(
				&witness)) {
		...
		cerr << "ERROR: in "
				"CliqueNumberSimpleGraphs_AtLeast_DynamicCore::forget_v cast "
				"error"
			 << endl;
		exit(20);
	}
...
WitnessSetPointer CliqueNumberSimpleGraphs_AtLeast_DynamicCore::join(
	const Bag &b, const Witness &witness1, const Witness &witness2) {
	if (const CliqueNumberSimpleGraphs_AtLeast_Witness *e1 =
			dynamic_cast<const CliqueNumberSimpleGraphs_AtLeast_Witness *>(
				&witness1)) {
		if (const CliqueNumberSimpleGraphs_AtLeast_Witness *e2 =
				dynamic_cast<const CliqueNumberSimpleGraphs_AtLeast_Witness *>(
					&witness2)) {
			// Note: Do not merge the two "if" instructions above into a single
			// "if". Potential for errors
			CliqueNumberSimpleGraphs_AtLeast_WitnessPointerConst w1 =
				e1->shared_from_this();
			CliqueNumberSimpleGraphs_AtLeast_WitnessPointerConst w2 =
				e2->shared_from_this();
			CliqueNumberSimpleGraphs_AtLeast_WitnessSetPointer witnessSet(
			...
			cerr << "ERROR: in "
					"CliqueNumberSimpleGraphs_AtLeast_DynamicCore::join cast "
					"error"
				 << endl;
			exit(20);
		}
	} else {
		cerr << "ERROR: in CliqueNumberSimpleGraphs_AtLeast_DynamicCore::join "
				"cast error"
			 << endl;
		exit(20);
	}
...
WitnessSetPointer CliqueNumberSimpleGraphs_AtLeast_DynamicCore::clean(
	WitnessSetPointer witnessSet) {
	if (CliqueNumberSimpleGraphs_AtLeast_WitnessSetPointer e =
			dynamic_pointer_cast<CliqueNumberSimpleGraphs_AtLeast_WitnessSet>(
				witnessSet)) {
		return clean_implementation(e);
	} else {
		cerr << "ERROR: in CliqueNumberSimpleGraphs_AtLeast_DynamicCore::clean "
				"cast error"
			 << endl;
		exit(20);
	}
...
bool CliqueNumberSimpleGraphs_AtLeast_DynamicCore::is_final_witness(
	...
	if (const CliqueNumberSimpleGraphs_AtLeast_Witness *e =
		...
		cerr << "ERROR: in "
				"CliqueNumberSimpleGraphs_AtLeast_DynamicCore::is_final_"
				"witness cast error"
			 << endl;
		exit(20);
	}
...
std::map<std::string, std::string> *metadata() {
	return new std::map<std::string, std::string>(
		CliqueNumberSimpleGraphs_AtLeast_DynamicCore().getAttributes());
}
}

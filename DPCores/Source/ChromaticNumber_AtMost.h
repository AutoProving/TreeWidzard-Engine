...
typedef shared_ptr<ChromaticNumber_AtMost_Witness>
	ChromaticNumber_AtMost_WitnessPointer;
typedef shared_ptr<ChromaticNumber_AtMost_Witness const>
	ChromaticNumber_AtMost_WitnessPointerConst;
class ChromaticNumber_AtMost_Witness
	: public Witness,
	  public enable_shared_from_this<ChromaticNumber_AtMost_Witness> {
  public:
	// std::map<unsigned,unsigned> coloring; //The first coordinate of each pair
	// is a vertex and the second a color.
...
class ChromaticNumber_AtMost_WitnessSet
	: public WitnessSetTypeTwo<ChromaticNumber_AtMost_WitnessSet> {
	shared_ptr<WitnessSet> createEmptyWitnessSet() override;
};

typedef shared_ptr<ChromaticNumber_AtMost_WitnessSet>
...
class ChromaticNumber_AtMost_DynamicCore : public DynamicCore {
	...
	void intro_v_implementation(
		unsigned i, const Bag &b, ChromaticNumber_AtMost_WitnessPointerConst w,
	...
	void join_implementation(
		const Bag &b, ChromaticNumber_AtMost_WitnessPointerConst w1,
		ChromaticNumber_AtMost_WitnessPointerConst w2,
		ChromaticNumber_AtMost_WitnessSetPointer witnessSet);
	ChromaticNumber_AtMost_WitnessSetPointer clean_implementation(
		ChromaticNumber_AtMost_WitnessSetPointer witnessSet);
